...
namespace WeChat.NET.HTTP
{
    ...
    class WXService
    {
        ...
        // 定义一个静态变量来保存类的实例
        private static WXService uniqueInstance;
        // 定义一个标识确保线程同步
        private static readonly object locker = new object();

        private static JObject initInfo = null;
        ...
        public static WXService GetWXService()
        {
            ...
            if (uniqueInstance == null)
            {
                lock (locker)
                {
                    ...
                    if (uniqueInstance == null)
                    {
                        uniqueInstance = new WXService();
                        initInfo = uniqueInstance.WxInit();
                    }
                }
            }
